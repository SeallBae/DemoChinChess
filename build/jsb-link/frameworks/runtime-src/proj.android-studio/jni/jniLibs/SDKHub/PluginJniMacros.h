...
#define CALL_BASE_RET_JAVA_FUNC_WITH_PARAM(retType, paramCode, param, retCode, defaultRet)                    \
...
#define CALL_BASE_RET_JAVA_FUNC(retType, paramCode, retCode, defaultRet)                                      \
retType ret = defaultRet;                                                                                     \
return_val_if_fails(funcName != NULL && strlen(funcName) > 0, ret);                                           \
PluginJavaData* pData = PluginUtils::getPluginJavaData(thiz);                                                 \
return_val_if_fails(pData != NULL, ret);                                                                      \
...
#define CALL_JAVA_FUNC_WITH_VAL_LIST(retCode)                                                                 \
...
if (0 == nParamNum) {                                                                                         \
    ...
    switch(pRetParam->getCurrentType()) {                                                                     \
        ...
        case PluginParam::kParamTypeStringMap: {                                                              \
            ...
            paramCode = "(Ljava/util/Hashtable;)";                                                            \
            paramCode.append(jRetCode);                                                                       \
    ...
    if (needDel) { delete pRetParam; pRetParam = NULL; }                                                      \
}                                                                                                             \
return ret;                                                                                                   \
